...
## Open items
1. **Cal.com is not live.** The embed container `#my-cal-inline-systemize-discovery` and its card are final on all three booking sections, but no `calLink` was supplied. Paste the complete Cal.com inline snippet when it arrives: Devon's calendar for the homepage and Calibration, the cofounder's separate link for 3PL. Pass industry context as booking metadata (`config: { metadata: { industry: "calibration" | "3pl" } }`) so leads arrive tagged.
